...
class MyHTMLParser(HTMLParser):
    ...
    def parse_data(self):
        ...
        def reallocate_parent(comp):
            if comp.children is not None and len(comp.children) >= 1:
                for child in comp.children:
                    reallocate_parent(child)
                if len(comp.children) == 1 and comp.children[0].type != "data":
                    comp.parent.children = comp.children
            return

        self.thisParsed = MyHTMLComponent("root", None, None, None)
        ...
        #reallocate_parent(self.thisParsed)
